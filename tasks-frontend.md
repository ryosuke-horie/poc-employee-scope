...
【仕様（Codex）】（仕様書: `docs/frontend/navigation-routing-spec.md`）
- [x] URLクエリ I/F 仕様を定義（対象=フィルタ/ソート/ページング）
  - パラメータ名・型・デフォルト・バリデーション・無効値時の挙動
  - シリアライズ/デシリアライズ方針（複数値、数値/範囲、小数精度、並び順）
  - 共有可能な安定リンクの要件（未知キーの無視、後方互換）
- [x] `/company/[id]` 深いリンク時の状態維持仕様
  - 一覧→詳細→戻る/進むの際に保持する状態の範囲（検索語/フィルタ/ソート/ページング/選択）
  - 履歴スタックとURL更新の期待動作（push/replace の使い分け、スクロール位置）
- [x] 選択ハイライトとキーボード移動（↑/↓）の仕様
  - フォーカス管理（roving tabindex/ARIA ロール）とハイライトの定義
  - 折り返し/ページ跨ぎ/フィルタ変更時の選択リセット方針

【実装（Claude）】
- [ ] 一覧のURLクエリ同期を実装（App Router）
  - `useSearchParams`/`router.replace` を用い、入力操作ごとの履歴汚染を防止
  - 仕様に沿ったクエリのデ/シリアライズと型ガードを実装
- [ ] 詳細ページの状態維持/復元を実装
  - 一覧の状態をクエリで持ち回り、戻る/進むで同一状態を再現
  - 共有リンクからの直接流入でも期待表示になることを確認
- [ ] 選択ハイライト/キーボード移動を実装
  - `selectedCompanyId` を状態として管理し、↑/↓で前後に移動
  - フィルタ/ソート変更時の選択整合性とスクロール追従を考慮

受入（ナビ/ルーティング）
- 共有リンクを開くと一覧条件と選択が復元され、詳細リンクはその状態を保持（仕様書準拠）。
- 戻る/進むで一覧状態（クエリ、スクロール、選択）が一致し、意図しない履歴汚染がない。
- 検索タイピング/微調整は `replace`、ページング/選択変更は `push` が適用される。
- 無効なクエリは既定へフォールバックし、既知キーのみ反映（警告UIは不要）。
- キーボード: ↑/↓で選択移動、Enterで詳細遷移し、`aria-selected`/ロールが更新、選択行は視界内にスクロール。
