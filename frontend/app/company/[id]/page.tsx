...
export default function CompanyDetailPage() {
  ...
  const handleSave = async () => {
    ...
    if (reviewData) {
      const testData = {
        ...reviewData,
        review_state: reviewData.review_state.map(r => 
          r.company_id === companyId ? localReviewState : r
        )
      };
      
      // 新規の場合は追加
      if (!reviewData.review_state.find(r => r.company_id === companyId)) {
        testData.review_state.push(localReviewState);
      }
      
      const validationResult = await validateReview(testData);
      if (!validationResult.valid) {
        setValidationErrors(validationResult.errors || []);
