...
## 抽出ロジック（正規表現とLLMの使い分け）

- 正規表現の条件:
  - ページ本文テキストに対し、従業員数に関する複数パターンでマッチを実施（例: `従業員数\s*([0-9,，０-９]+)\s*(名|人)` など）。
  - マッチ候補から最も信頼度の高いものを選択し、数値に正規化（全角→半角、カンマ除去）。
  - 妥当性チェック: 1〜10,000,000 の範囲内で数値として解釈できること。
  - 上記を満たす場合は正規表現の結果を採用（この時点で抽出完了）。
- LLMフォールバックの条件:
  - 正規表現の候補が見つからない、または数値として妥当でない場合。
  - `.env` に OpenRouter API キーが設定されている場合のみ実行。
  - 入力テキストは従業員関連の行を優先して抽出し、長さを制限した上で送信。
  - LLMの出力は JSON で受け取り、数値化と範囲チェック（1〜10,000,000）を通過した場合に採用。
- ログ出力（標準出力）:
  - 正規表現成功時は「正規表現で抽出成功」として人数・信頼度・スニペットを表示。
  - 正規表現で見つからない場合は「正規表現で見つからないためLLMを使用」と表示。
  - LLM成功時は「LLMで抽出成功」として人数・信頼度・モデル名を表示。
  - 企業処理のサマリ行では、抽出手法 `[regex|llm]` と最終人数をあわせて表示。
